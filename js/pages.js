...
function showPage(page) {
  ...
  if (page === 'welcome') {
    main.innerHTML =
      ...
      + '<div class="welcome-card" style="border-color:rgba(160,100,255,0.35);box-shadow:0 0 10px rgba(160,100,255,0.12)"><h3 style="color:#c084fc">\ud83c\udfc6 '+t('nav.ranking')+'</h3><p>'+(currentLang==='en'?'Explore the Top 6 Pok\u00e9mon of each type with counter-strategies and build recommendations.':'Przegl\u0105daj Top 6 Pok\u00e9mon\u00f3w ka\u017cdego typu z kontrstrategiami i rekomendacjami build\u00f3w.')+'</p><p style="margin-top:6px"><button class="mc-btn" onclick="showPage(\'ranking\')" style="background:rgba(70,40,120,0.75);border-color:rgba(160,100,255,0.3)">\u2192 '+t('nav.ranking')+'</button></p></div>'
      + '<div class="welcome-card" style="border-color:rgba(255,80,80,0.3);box-shadow:0 0 10px rgba(255,80,80,0.12)"><h3 style="color:var(--red)">\u2694 '+t('nav.battle')+'</h3><p>'+t('battle.welcomeDesc')+'</p><p style="margin-top:6px"><a class="mc-btn" href="arena.html" style="background:rgba(140,40,40,0.75);border-color:rgba(255,80,80,0.3);text-decoration:none">\u2192 '+t('nav.battle')+'</a></p></div>'
      + '</div>';
  }
